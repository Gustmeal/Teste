...
class Mensagem(db.Model):
    __tablename__ = 'DCA_TB011_MENSAGENS'
    __table_args__ = {'schema': 'DEV'}  # Mesmo esquema das outras tabelas

    ID = db.Column(db.Integer, primary_key=True, autoincrement=True)
    REMETENTE_ID = db.Column(db.Integer, db.ForeignKey('DEV.DCA_TB000_USUARIOS.ID'), nullable=False)
    DESTINATARIO_ID = db.Column(db.Integer, db.ForeignKey('DEV.DCA_TB000_USUARIOS.ID'), nullable=False)
